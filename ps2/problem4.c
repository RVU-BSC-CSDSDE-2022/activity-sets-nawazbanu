...
Kalakand input_kalakand(){
  ...
  scanf("%d %d %d",&k.quantity_of_milk,&k[i].quantity_of_sugar,&k[i].weight_of_dry_fruits);
  return k;
}
...
float find_weight(Kalakand k){
  if(weight <= 5){
    perimeter = r.quantity_of_milk + r.quantity_of_sugar;
    }
  else{
    perimeter = pow((r.quantity_of_milk),r.quantity_of_sugar);
  }
  return weight;
  }
...
void output(int n, Kalakand k[n], float weight){
  printf("The sum of kalakands is %f",weight);
}